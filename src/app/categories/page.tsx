    ...
    <main className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <div className="flex">
        <aside className="w-28 sm:w-40 shrink-0 border-r border-gray-100 dark:border-gray-800">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActive(cat)} className={active === cat ? "w-full text-left px-3 py-4 text-xs sm:text-sm font-semibold border-l-2 border-brand bg-gray-50 dark:bg-gray-900 text-black dark:text-white" : "w-full text-left px-3 py-4 text-xs sm:text-sm text-gray-600 dark:text-gray-400 border-l-2 border-transparent"}>
              {cat}
            </button>
          ))}
        </aside>
        ...
        <div className="flex-1 px-4 py-4">
          <div className="border border-gray-100 dark:border-gray-800 rounded-lg p-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {subcats.map((sub) => (
                <a key={sub} href={"/shop?category=" + encodeURIComponent(active)} className="flex flex-col items-center gap-2">

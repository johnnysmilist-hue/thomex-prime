  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-3 gap-6">
      {tiles.map((tile) => (
        <div
          key={tile.title}
          className={`rounded-lg p-6 h-40 flex flex-col justify-between ${
            tile.dark ? "bg-brand-dark text-white" : "bg-brand-light/20 text-brand-dark"
          }`}
        >
          <div>
            ...
            href="/shop"
            className={`inline-block w-fit px-4 py-2 rounded-md text-sm font-semibold ${
              tile.dark ? "bg-brand" : "bg-brand text-white"
            }`}
          >
            {tile.cta}
          </a>
